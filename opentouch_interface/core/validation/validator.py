...
class SessionStateManager:
    ...
    def get_group_registry(self) -> Union[GroupRegistry, MockGroupRegistry]:
        return self.state or MockGroupRegistry()


class GroupNameConfig(BaseModel):
    group_name: str = Field(default=None, description="Defaults to 'Group <current-group-count>'.")
    "Group name."

    @field_validator('group_name', mode='before')
    def set_default_group_name(cls, v):
        return v or f'Group {SessionStateManager().get_group_registry().group_count}'
